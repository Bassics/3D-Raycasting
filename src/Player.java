...
public class Player {
    ...
    public void moveSideways(float dir) {
        ...
        float posX = position.x;
        float posY = position.y;
        if (currentMap[(int)(position.x + newDirection.x * moveSpeed * dir)][(int)position.y] == 0)
            posX = position.x + newDirection.x * moveSpeed * dir;
        if (currentMap[(int)position.x][(int)(position.y + newDirection.y * moveSpeed * dir)] == 0)
    ...
    public void tiltCamera(float dir) {
        if (Math.abs(upWard + (rotSpeed * dir)) < 1)
            upWard += rotSpeed * dir;
    }
